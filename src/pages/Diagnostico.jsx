/**
 * PÁGINA: Diagnóstico Financeiro
 * DESCRIÇÃO: Análise detalhada da saúde financeira com gráficos de distribuição e evolução.
 * ---------------------------------------------------------
 */
...
export default function Diagnostico() {
  ...
  /* 
     CARREGAMENTO DE DADOS: Busca dados do mês atual e histórico para os gráficos
     -------------------------------------------------------
  */
  useEffect(() => {
      ...
      if (user) {
        try {
          ...
          if (docSnap.exists()) {
            ...
            // Processamento dos dados do mês atual para o Score e Distribuição
            if (monthData) {
              ...
              // Cálculo de Score: Baseado na porcentagem de comprometimento da renda
            ...
            // Processamento do Histórico para o Gráfico de Evolução
            if (data.historicoMensal) {
              const history = Object.entries(data.historicoMensal)
                .map(([month, mData]) => {
                  const ent = (Number(mData.rendaBase) || 0) + 
                              (mData.rendasExtras?.reduce((acc, curr) => acc + (Number(curr.valor) || 0), 0) || 0);
                  const sai = (mData.despesas?.reduce((acc, curr) => acc + (Number(curr.valor) || 0), 0) || 0) +
                              (mData.dividas?.reduce((acc, curr) => acc + (Number(curr.parcela) || 0), 0) || 0);
                  return { month, entradas: ent, saidas: sai };
                })
                .sort((a, b) => a.month.localeCompare(b.month)); // Ordenação cronológica simples
  ...
  if (loading) {
    return (
      <div className="h-screen flex items-center justify-center bg-app-background">
    ...
    <div className="min-h-screen bg-app-background text-on-surface p-6 pb-32 md:pb-8 transition-colors duration-300">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center gap-4 mb-8">
          <button onClick={() => navigate("/home")} className="h-12 w-12 bg-surface-low dark:bg-surface-high border border-default rounded-2xl shadow-sm dark:shadow-none transition md:hidden flex items-center justify-center">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          {/* COLUNA ESQUERDA: Score e Resumo Financeiro */}
          <div className="lg:col-span-4 space-y-6">
            
            {/* CARD: Score de Saúde Financeira */}
            <div className="card-diag border border-default p-8 rounded-[32px] shadow-xl dark:shadow-none text-center">
              <div className="relative inline-block mb-6">
                <svg className="w-32 h-32 transform -rotate-90">
                  ...
                  <circle cx="60" cy="60" r="54" fill="none" stroke="currentColor" strokeWidth="10" strokeDasharray={339.29} strokeDashoffset={339.29 - (339.29 * score) / 100} strokeLinecap="round" className="text-info transition-all duration-1000" />
                </svg>
            ...
            {/* CARD: Resumo Numérico do Mês */}
            <div className="card-diag border border-default p-8 rounded-[32px] shadow-xl dark:shadow-none">
          ...
          {/* COLUNA DIREITA: Visualização de Gráficos (Distribuição vs Evolução) */}
          <div className="lg:col-span-8 space-y-6">
            
            {/* SELETOR DE ABAS: Alterna entre os tipos de análise */}
            <div className="flex p-1.5 bg-surface-low dark:bg-surface-high border border-default rounded-2xl shadow-sm dark:shadow-none">
              {[
                ...
                <button 
                  ...
                  className={`flex-1 flex items-center justify-center gap-3 py-3 rounded-xl text-[10px] font-black transition-all duration-300 ${activeTab === tab.id ? "bg-on-surface text-surface-lowest dark:bg-white dark:text-black shadow-lg scale-[1.02]" : "text-on-surface-variant hover:text-on-surface"}`}
                >
                  {tab.icon}
            ...
            {/* CONTAINER DO GRÁFICO: Renderiza o conteúdo baseado na aba ativa */}
            <div className="card-diag border border-default p-8 rounded-[32px] shadow-xl dark:shadow-none min-h-[400px]">
              {activeTab === "distribuicao" ? (
                /* ABA: Distribuição de Renda (Gráfico de Pizza/Donut) */
                <>
                ...
                /* ABA: Evolução Mensal (Gráfico de Área) */
                <MonthlyEvolution monthlyData={historicalData} />
              )}
            </div>

            {/* BOTÃO DE AÇÃO: Link para o plano estratégico */}
            <button onClick={() => navigate("/plano")} className="w-full btn-diag-primary py-5 rounded-[24px] font-black text-xs uppercase tracking-[0.2em] shadow-xl hover:scale-[0.98] transition-transform active:scale-95">
